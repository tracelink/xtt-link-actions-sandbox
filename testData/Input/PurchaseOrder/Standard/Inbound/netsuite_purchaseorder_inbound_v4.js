    ...
    function queryPurchaseOrders({ linkActionContext, cursor }) {
        try {
            ...
            const payload = JSON.stringify({
                "q": `select t.id, TO_CHAR( t.lastmodifieddate, '${dateTimeFormat}' ) from transaction t where t.type='PurchOrd' and t.status='B' and SYS_EXTRACT_UTC(t.lastmodifieddate)>TO_TIMESTAMP('${dateTime}', '${dateTimeFormat}' ) order by t.lastmodifieddate`
            });
