export function pollForDataInbound(linkActionContext, cursorString) {
    let cursor = null;
    if (cursorString) {
    ...
    //const statuses = ['Pending Receipt'];
    ...
    function queryPurchaseOrders(linkActionContext, cursor) {
        try {
            ...
            const dateTime = epochToFormattedDateTime(dateTimeFromCursor, dateFormat, timeFormat);
            const header = { 'Prefer': 'transient' };
            const payloadJson = {
                q: "select t.id, TO_CHAR( t.lastmodifieddate, 'YYYY-MM-DD HH:MI:SS AM' ) from transaction t where t.type='PurchOrd' and t.status='B' and SYS_EXTRACT_UTC(t.lastmodifieddate)>TO_TIMESTAMP('2024-03-15 1:54:00 AM', 'YYYY-MM-DD HH:MI:SS AM' ) order by t.lastmodifieddate"
            }
            const payload = JSON.stringify(payloadJson);
            linkActionContext.api.log(linkActionContext, 'LinkActionContext:');
            linkActionContext.api.log(linkActionContext, JSON.stringify(linkActionContext));
            linkActionContext.api.log(linkActionContext, 'URL:');
            linkActionContext.api.log(linkActionContext, url);
            linkActionContext.api.log(linkActionContext, 'Payload:');
            linkActionContext.api.log(linkActionContext, payload);
            const response = linkActionContext.api.oAuth2.post(linkActionContext, `${url}`, header, payload);
            linkActionContext.api.log(linkActionContext, "POST RESPONSE:");
            linkActionContext.api.log(linkActionContext, JSON.stringify(response, null, 2));
    ...
    function fetchPurchaseOrder(linkActionContext, data) {
        try {
            const uri = `${linkActionContext.config.url}/${netsuiteRecordType}/${data.id}`
            const header = {};
            ...
            linkActionContext.api.log(linkActionContext, 'Full Purchase Order:');
            linkActionContext.api.log(linkActionContext, JSON.stringify(purchaseOrder, null, 2));
    ...
    function epochToFormattedDateTime(epochTime, dateFormat, timeFormat) {
        const date = new Date(epochTime);
        ...
        const isAm = timeFormat.toLocaleUpperCase().includes('AM');
        const isPM = timeFormat.toLocaleUpperCase().includes('PM');
    ...
    function main() {
        linkActionContext.api.log(linkActionContext, 'START, CONFIG VALUE IS: ');
        try {
            linkActionContext.api.log(linkActionContext, linkActionContext.config.key1);
        } catch (e) {
            linkActionContext.api.log(linkActionContext, 'ERROR');
            linkActionContext.api.log(linkActionContext, e);
        }
        linkActionContext.api.log(linkActionContext, 'END');
        ...
        if (purchaseOrders) {
            ...
            linkActionContext.api.log(linkActionContext, 'Fetching each PO...');
            //            linkActionContext.api.log(linkActionContext, JSON.stringify(purchaseOrders,null,2));
            ...
            purchaseOrders.items.forEach(po => {
                if (!isFailed) {
                    ...
                    // send data to B2B
                    const purchaseOrderString = JSON.stringify(purchaseOrder);
                    const cursorString = JSON.stringify(cursor);
                    const b2bResponse = linkActionContext.api.sendDataInbound('PurchaseOrder', purchaseOrderString, cursorString, '');
                }
