...
export function pollForDataInbound(linkActionContext, cursor) {
    ...
    function queryPurchaseOrders(linkActionContext, cursor) {
        try {
            ...
            const dateTimeFromCursor = cursor.fetchStartTime;
            const dateTime = convertFromEpoch(dateTimeFromCursor, dateFormat, timeFormat);
            const header = {};
            //const dateTime = cursor.lastSuccessTime;
            //const dateTime = "01/01/2024 01:15 am";
            const response = linkActionContext.api.oAuth2.get(linkActionContext, `${url}/purchaseOrder?q=lastModifiedDate AFTER "${dateTime}"`, header);
            //const response = linkActionContext.api.oAuth2.get(linkActionContext, `${url}/purchaseOrder?q=tranId is 8`, header);
    ...
    function fetchPurchaseOrder(linkActionContext, data) {
        try {
            ...
            if (!statuses.includes(purchaseOrder.status.id)) {
                linkActionContext.api.log(linkActionContext, 'Skipping::Purchase Order status is ' + purchaseOrder.status.id)
                return;
            }
        ...
        if (purchaseOrders) {
            ...
            purchaseOrders.items.forEach(po => {
                ...
                if (purchaseOrder) {
                    ...
                    cursor.executionTimeInMillis = Date.now() - fetchStartTime;
                }
                else {
                    cursor.lastFailed = {
                ...
                // send data to B2B
                return linkActionContext.api.sendDataInbound('PurchaseOrder', purchaseOrder, cursor, '');
            });
        }
        else {
